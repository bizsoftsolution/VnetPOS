Public Class frmPostage
    ...
    Function FormValidate() As Boolean
        ...
        If cmbPaymentType.Text.Trim() = "" Then
            ErrorProvider1.SetError(cmbPaymentType, "Please Enter the Payment Type")
            cmbPaymentType.Focus()
            RValue = False
        Else
            If cmbPaymentType.FindStringExact(cmbPaymentType.Text) = -1 Then
                ErrorProvider1.SetError(cmbPaymentType, "Please Enter the Valid Payment Type")
                cmbPaymentType.Focus()
                RValue = False
            Else
    ...
    Private Sub CTRL_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCode.Enter, txtTotalDiscount.Enter, txtTotalAmount.Enter, txtSearchBox.Enter, txtRoundOff.Enter, txtNarration.Enter, txtItemsAmount.Enter, txtId.Enter, txtExtras.Enter, dtpDate.Enter, cmbPaymentType.Enter, txtSNo.Enter, txtRate.Enter, txtQty.Enter, txtTotAmount.Enter, cmbProductName.Enter, txtAddress.Enter, cmbGSTPer.Enter, txtGSTAmt.Enter, txtDisPer.Enter, txtDisAmt.Enter, txtTotGSTAmt.Enter, cmbCode.Enter, txtOrderNo.Enter, txtICNo.Enter, txtMemberName.Enter, txtPhoneNo.Enter, txtMemberId.Enter
    ...
    Private Sub frm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        db.Postage.SetControls(txtId, txtCode, txtOrderNo, dtpDate, cmbPaymentType, txtExtras, txtTotalDiscount, txtNarration, txtMemberName, txtAddress, txtMemberId, txtICNo, txtPostalCode, txtPhoneNo)
        db.PostageDetails.SetControls(txtDetailsId, txtDetailsCode, txtMasterCode, txtProductCode, txtRate, txtQty, cmbGSTPer, txtDisAmt, cmbCode)
    ...
    Private Sub dgvDetails_RowsAdded(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewRowsAddedEventArgs) Handles dgvDetails.RowsAdded
        If dgvDetails.Rows.Count > 9 Then
            dgvDetails.Width = 1036
        Else
            dgvDetails.Width = 1028
        End If
    End Sub
    ...
    Sub NewForm()
        ...
        NewItem()
        db.Postage.NewRecord()
        'cmbMemberId.Focus()
        lbStatus.Text = ""
        txtExtras.Text = 0
        txtAddress.Text = ""
        txtMemberId.Text = ""
        cmbPaymentType.Text = "Online Banking"
        Dim RefNo As String
        RefNo = db.Postage.Data("Max(RefNo)")
        If RefNo <> "" Then
            txtOrderNo.Text = RefNo + 1
        Else
            txtOrderNo.Text = "1000"
        End If
    End Sub
    Sub NewItem()
        ItemState = "New"
        db.PostageDetails.NewRecord()
    ...
    Sub ViewForm()
        ...
        Else
            Dim str As String = ""
            str = db.Postage.ViewRecordByCode(txtSearchBox.Text)
            If str <> "" Then
                MsgDialog.ShowMsgDlg(str, Me.Text, "I")
            Else
                txtSearchBox.Text = ""
                Dim dv As New DataView
                dv = db.ViewPostageDetails.Grid("*", "PostageCode='" + txtCode.Text + "'")
                dgvDetails.Rows.Clear()
    ...
    Sub FindTotalAmount()
        ...
        If cmbPaymentType.Text = "Credit Card" Then
    ...
    Sub SaveForm()
        If FormValidate() Then
            Dim str As String = ""
            str = db.Postage.SaveRecord()
            If str.Trim() <> "" Then MsgDialog.ShowMsgDlg("Postage Sent To Invoice", Me.Text, "I")
            If str.ToLower.Trim() = "inserted" Or str.ToLower.Trim() = "updated" Then

                db.PostageDetails.DeleteRecordByUserCode(txtCode.Text)
                For i As Integer = 0 To dgvDetails.Rows.Count - 1
                    If dgvDetails.Item("SNo", i).Value = Nothing Then Exit For
                    db.PostageDetails.NewRecord()
                    ViewItem(i)
                    db.PostageDetails.SaveRecord()
    ...
    Private Sub txtMId_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtMemberId.TextChanged
        Try
            If txtMemberId.Text <> "" Then
                ...
                lbStatus.Text = db.ViewMemberRegistration.Data("MemberStatus", String.Format("MemberCode='{0}'", txtMemberId.Text))
                If lbStatus.Text = "Active" Then
